...
class GameEngine {
  ...
  async start() {
    try {
      ...
      console.error('❌ Error starting GameEngine:', error);
    }
  }
  ...
  async newRound() {
    try {
      ...
      const countdownInterval = setInterval(async () => {
        ...
        if (remaining > 0) {
          try {
            await rtdb.ref('currentRound/countdownSec').set(remaining);
            this.broadcast({ type: 'WAITING', countdownSec: remaining });
          } catch (e) {
            console.error('❌ RTDB Countdown update failed:', e.message);
          }
        } else {
    ...
    try {
      ...
      this.tickInterval = setInterval(async () => {
        ...
        if (multiplier >= this.currentRound.crashAt) {
          ...
          try {
            await rtdb.ref('currentRound/multiplier').set(multiplier);
            this.broadcast({ type: 'TICK', multiplier });
          } catch (e) {
            // Silently fail ticks to avoid log spam, but log if it's a persistent connection issue
            if (multiplier % 1 === 0) console.error('❌ RTDB Tick update failed:', e.message);
          }
  ...
  async handleAutoCashout(cashout) {
    ...
    try {
      await db.runTransaction(async (t) => {
        ...
        const newBalance = (userData.walletBalance || 0) + winnings;
        t.update(userRef, {
      ...
      await rtdb.ref(`rounds/${this.currentRound.roundId}/cashouts/${uid}`).set({
        multiplier,
        winnings,
      ...
      console.error(`❌ Auto-cashout failed for user ${uid}:`, e);
    }
  }
  ...
  getCurrentMultiplier() { return this.currentRound.multiplier; }
  getRoundState() { return this.currentRound.state; }
  getRoundId() { return this.currentRound.roundId; }
}
